...
public:
  ...
  void get_data();
  TH1F* histo_for_stack(bool is_signal, const char* variable_name, const char* selection, const char* bins, const char* x_min, const char* x_max, int fill_colour, bool is_cut,const char* x_minc_nocut, const char* x_maxc_nocut);
  void set_histo_style(bool is_signal, int fill_colour = 0);
  TH1F* draw_data(const char* variable_name, const char* selection, const char* bins, const char* x_min, const char* x_max, const char* x_minc_nocut, const char* x_maxc_nocut);
  double get_data_error(TH1F* hist, int bin);
  TH1F* set_error_bars(TH1F* hist);
  const char* scale_bins_for_cut(const char* bins, const char* x_minc_nocut, const char* x_maxc_nocut, const char* x_minc_cut, const char* x_maxc_cut);
