...
void draw_stacked_histoplots(std::vector<DataTree*> data_trees, char* variable_name, TCut cut) {
  ...
  for(int i = 0; i < data_trees.size(); i++) {
    TH1F* single_histo = data_trees[i]->create_histo_for_stack(colours[i], variable_name, cut);
