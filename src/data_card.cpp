                                 ...
                                 Variable* var, bool with_cut, std::vector<Variable*>* variables)
{
  ...
  double rates = get_rates(data, sbg_chains,  bg_chain,
                                 var, bool with_cut, std::vector<Variable*>* variables);
  for(int i=0; i<bg_chains.size();i++){
    ...
    bg_errors[i] = 1+(bg_errors_no/(N*weight));
  }
  return bg_errors;
}
                                 ...
                                 Variable* var, bool with_cut, std::vector<Variable*>* variables)
{
  ...
  for(int i=0; i<bg_chains.size();i++){
    double weight = MCWeights::calc_mc_weight(data, bg_chains, bg_chain, var, with_cut, variables);
    double N = get_histo_integral(bg, with_cut, var);
    rates[i]=N*weight;
  }
}
