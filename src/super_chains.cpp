...
SuperChains::SuperChains()
{
		...
		bg_zjets_vv   = new DataChain(zjets_vv, zjets_vv_label, zjets_vv_legend, "&&(nselmuons == 2)&&(m_mumu>60)&&(m_mumu<120)");
		bg_qcd        = new DataChain(qcd, qcd_label, qcd_legend);
		signal_chain  = new DataChain(mc_signal_data, mc_signal_label, mc_signal_legend);
		data_chain    = new DataChain(data, data_label, data_legend, "&&(nvetomuons == 0)");
}
